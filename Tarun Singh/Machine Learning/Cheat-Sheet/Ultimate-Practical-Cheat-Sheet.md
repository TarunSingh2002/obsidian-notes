...
#### Frame
- Answer 3 questions 
	- what is target columns?
	- Regression or classification problem?
	- what metric?
		- For Regression -> RSME (Default metrics) 
		- For balance classification -> Accuracy, confusion matrix
		- For imbalance classification -> F1, Roc-Auc / Use **recall** if missing a positive is costly (fraud, disease); **precision** if false alarms are costly
#### Peek
- Answer 
	- shape 
...
s= SimpleImputer(strategy='mean/median/constant/most_frequent', fill_value='used when stategy is contant', add_indicator=True/False)
...
#### Handling Outliers
- Tree based models basically ignore outliers but linear models, knn, svm and anything with distance or gradients require the outliers handling
``` 
...
- Generally choice Capping, instead of Trimming
```
...
#### Transform skew (optional, linear models only)
Trees don't care about skew → skip for trees
...
#### Feature Scaling
- Trees don't care about scaling → skip for trees
- StandardScaler is your one default. 
 - Types :
	- Standardization
